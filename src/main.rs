...
fn task1(){
    ...
    loop {
        ...
        if user_payment < random_price {
            ...
            let mut change :f32 = change as f32/ 100.0;
            if change >0.00 {
                let mut changer: [f32; 8] = [0.0; 8];
                let currency: [f32; 8] = [2.0, 1.0, 0.5,0.2, 0.1, 0.05, 0.02,0.01];
                for ch in (0..7) {
                    changer[ch] = change%currency[ch];
                    println!("Change : {} Changer : {} Currency : {}",change,changer[ch],currency[ch]);
                    change =change-(changer[ch]*currency[ch]);
                    println!("Change : {} Changer : {} Currency : {}",change,changer[ch],currency[ch]);
                }
